...
const HomePage = () => {
  ...
  const [suggestions, setSuggestions] = useState<Actor[]>([]); // Specify Actor[] as the type
  const [selectedActor, setSelectedActor] = useState<Actor | null>(null);
  const [loading, setLoading] = useState<boolean>(false);
  const [isFocused, setIsFocused] = useState<boolean>(false); // Keep isFocused state
  ...
  return (
    <div className="actors-page mx-auto max-w-3xl p-6">
      ...
      <div className="relative max-w-lg mx-auto" ref={searchRef}>
        ...
        {/* Display suggestions when the input is focused or has content */}
        {suggestions.length > 0 && (
          <ul className="absolute left-0 w-full mt-1 bg-gray-100 border border-gray-300 rounded-lg shadow-md z-10 flex flex-wrap gap-2 p-2">
            {suggestions.map((actor) => (
              <li
                key={actor._id}
                onClick={() => handleSelectActor(actor._id)}
